...
function initLogin() {
  document.getElementById("login-google")?.addEventListener("click", () => {
    ...
    // For POC: simulate with a prompt
    const token = prompt("Google access_token (POC):");
    if (!token) return;
    handleSocialLogin(() => API.loginGoogle(token));
  });

  document.getElementById("login-apple")?.addEventListener("click", () => {
    const access = prompt("Apple access_token (POC):");
    const id = prompt("Apple id_token (POC):");
    if (!access || !id) return;
    handleSocialLogin(() => API.loginApple(access, id));
  });
}
