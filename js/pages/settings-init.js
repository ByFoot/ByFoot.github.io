// pages/settings-init.js — init + event logic (full rewrite)
...
async function initSettings() {
  ...
  // ── Enable push button ────────────────────────────────────────────────────
  // This is the ONLY place where promptPermission:true is safe — it fires
  // directly from a user tap, satisfying Safari's gesture requirement.
  document.getElementById("enable-push-btn")?.addEventListener("click", async () => {
    const msgEl = document.getElementById("push-status-msg");
    msgEl.textContent = "…"; msgEl.className = "success-msg";
    console.log("[Settings] enable-push-btn clicked — calling initPushNotifications(promptPermission:true)");
    ...
    if (perm === "granted" && window.APP.pushToken) {
      msgEl.textContent = "✓ Notifications enabled";
    } else if (perm === "denied") {
      msgEl.textContent  = "Blocked — reset in iOS Settings > Safari";
      msgEl.className    = "error-msg";
    } else {
      msgEl.textContent = "Permission: " + perm;
    }
  });
  ...
  if (themeToggle) {
    // Sync initial state (page may have been re-rendered since last toggle)
    themeToggle.checked = getCurrentTheme() === "dark";

    themeToggle.addEventListener("change", () => {
      toggleTheme();
      // Update the label icon inline without re-rendering the whole page
      const isDark = getCurrentTheme() === "dark";
      const labelEl = document.getElementById("theme-toggle-label");
      if (labelEl) {
        const span = labelEl.previousElementSibling;
        if (span) {
          ...
          span.innerHTML = (isDark ? moonIcon : sunIcon) + "&nbsp;" +
  ...
  async function renderLocationDisplay(lat, lng) {
    ...
    if (!displayEl) return;
    if (lat == null || lng == null) { displayEl.textContent = t("settings.location_not_set"); return; }
    ...
    try {
      ...
      const label = [addr.road || addr.pedestrian, addr.neighbourhood || addr.suburb, addr.city || addr.town || addr.village].filter(Boolean).join(", ");
      if (label) displayEl.textContent = label;
    } catch { /* ignore — coords already shown */ }
  }
  ...
  document.getElementById("location-update-btn")?.addEventListener("click", async () => {
    ...
    if (navigator.permissions) {
      ...
      } catch { /* fall through — old browser will prompt via getCurrentPosition */ }
    }
  ...
  proEl.innerHTML = me.is_pro
    ...
    : '<ul class="pro-perks">' +
      ...
      '<div class="pro-buy-wrap">' +
        '<stripe-buy-button buy-button-id="buy_btn_1TWEySGTv1SoWf9cNt7zuEbz" publishable-key="pk_live_51SnY0EGTv1SoWf9cTiBdEXYvE8XHGuzbAdXgx2R4vfdo6vYMOryV2IJVqpOWmGkePkX70oVLIGxUl7RaGVqFO8s900K6OsUHSU"></stripe-buy-button>' +
      '</div>';
  ...
  // ── Staff logs button (only rendered for is_staff users) ───────────────────
  document.getElementById("staff-logs-btn")?.addEventListener("click", () => {
    showStaffLogs();
  });
